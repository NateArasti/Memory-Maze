...
public abstract class MazeGenerator
{
	...
	private MazeCell RemoveWallsWithBacktracker(MazeCell startCell)
	{
		...
		do
		{
			...
			if (unvisitedNeighbors.Count > 0)
			{
				...
				if (chosenCell.DistanceFromStart > furthest.DistanceFromStart &&
				    chosenCell.Neighbors.Values.Contains(null))
					furthest = chosenCell;
		...
		foreach (var neighbor in furthest.Neighbors.Where(neighbor => neighbor.Value == null))
		{
	...
	{
		foreach (var cell in cellA.Neighbors.Where(cell => cellB == cell.Value))
		{
			cellA.Walls[cell.Key] = false;
			break;
		}

		foreach (var cell in cellB.Neighbors.Where(cell => cellA == cell.Value))
		{
